...
public class MVPHud implements IGuiOverlay {
    ...
    /** MVP音乐名称显示组件 */
    private Component musicName = Component.empty();
    private Component userNm = Component.empty();  // 新增用户名字段
    ...
    // 新增字段用于存储玩家头像
    /** 玩家头像缓存文件 */
    private File playerAvatarFile;
    /** 玩家头像材质资源 */
    private ResourceLocation playerAvatarTexture;

    /**
     * 触发MVP动画
     * @param reason MVP数据，包含玩家信息等
     */
    public void triggerAnimation(MvpReason reason) {
        ...
        if (minecraft.level != null && minecraft.player != null) {
            minecraft.level.playLocalSound(
                    minecraft.player.getOnPos().above().above(),
                    flag ? FPSMSoundRegister.voice_ct_win.get() : FPSMSoundRegister.voice_t_win.get(),
        ...
        CompletableFuture.supplyAsync(() -> {
            return MvpMusicUtils.getMvpMusicName(reason.getPlayerName().getString());
        }).thenAccept(name -> {
            if (name != null) {
                this.musicName = Component.literal(name);
            }
        });

        // 异步获取用户名
        CompletableFuture.supplyAsync(() -> {
            return MvpMusicUtils.getUserNm(reason.getPlayerName().getString());
        }).thenAccept(name -> {
            if (name != null) {
                this.userNm = Component.literal(name);
            }
        });
    }
    ...
    public void render(ForgeGui gui, GuiGraphics guiGraphics, float partialTick, int screenWidth, int screenHeight) {
        ...
        if (roundBannerStartTime != -1) {
            ...
            if (bannerProgress >= 1f && mvpInfoStartTime == -1) {
                ...
                CompletableFuture.supplyAsync(() -> {
                    // 获取头像URL
                    String imageUrl = MvpMusicUtils.getMvpImageUrl(currentPlayerName.getString());
                    if (imageUrl != null) {
                        return MvpMusicUtils.downloadAndCacheImage(imageUrl);
                    }
                    return null;
                    ...
                    if (file != null) {
                        try {
                            ...
                            NativeImage nativeImage = NativeImage.read(Files.newInputStream(file.toPath()));

                            // 在主线程中注册材质
                            minecraft.execute(() -> {
                                this.playerAvatarFile = file;
                                this.playerAvatarTexture = minecraft.getTextureManager().register(
                                        "mvp_avatar",
                                        new DynamicTexture(nativeImage)
                                );
                            });
                        } catch (Exception e) {
                            FPSMatch.LOGGER.error("加载玩家头像时发生错误", e);
                        }
                    }
                });
            }
    ...
    private void renderMVPInfoPanel(GuiGraphics guiGraphics, PoseStack pose, float progress,
        ...
        // 构建完整的玩家名称
        Component fullPlayerName;
        if (!this.userNm.getString().isEmpty()) {
            fullPlayerName = Component.literal(this.userNm.getString() + " ")
                    .append(currentPlayerName);
        } else {
            fullPlayerName = currentPlayerName;
        }

        // 渲染完整的玩家名称
        renderScaledText(guiGraphics, pose, fullPlayerName,
                infoStartX,
                nameY,
                -1, nameScale);

        // 渲染音乐名称（位于玩家名称下方）
        int musicNameY = nameY + (int)(font.lineHeight * nameScale) + (int)(15 * scaleFactor);
        if (!this.musicName.getString().isEmpty()) {
            // 添加"音乐盒："前缀和"♪"后缀
            Component musicText = Component.literal("音乐盒："+this.musicName.getString() + " ♪");
            renderScaledText(guiGraphics, pose, musicText,
                    infoStartX,
                    musicNameY,
                    0xFFFFD700,  // 使用金色显示
                    scaleFactor * 1f);  // 使用标准缩放
        }

        // 渲染统计数据（在音乐名称下方）
        renderScaledText(guiGraphics, pose, extraInfo1,
                infoStartX,
                musicNameY + (int)(font.lineHeight * scaleFactor * 1.2f) + 4,
                -1,
                scaleFactor * 1.1f);
        ...
        renderScaledText(guiGraphics, pose, extraInfo2,
                infoStartX,
                currentY + (int)(font.lineHeight * scaleFactor * 1.4f) + 4,
                0xFFFFFFFF,
                scaleFactor * 1.1f);
    }
    ...
    private void renderAvatar(GuiGraphics guiGraphics, int x, int y, float scaleFactor) {
        ...
        // 优先使用下载的玩家头像
        if (this.playerAvatarTexture != null) {
            // 计算图片实际尺寸以保持正方形
            ...
            // 计算居中偏移，确保显示图片中心部分
            int xOffset = (textureWidth - size) / 2;
            int yOffset = (textureHeight - size) / 2;

            // 渲染正方形头像
            guiGraphics.blit(playerAvatarTexture,
                    x, y,  // 目标位置
                    scaledSize, scaledSize,  // 目标大小
                    xOffset, yOffset,  // 源图片偏移
                    size, size,  // 源图片大小
                    textureWidth, textureHeight  // 源图片总大小
            );
            return;
        }
    ...
    public void resetAnimation() {
        ...
        // 清理头像资源
        if (this.playerAvatarTexture != null) {
            minecraft.getTextureManager().release(this.playerAvatarTexture);
            this.playerAvatarTexture = null;
        }
        this.playerAvatarFile = null;

        // 清空音乐名称
        this.musicName = Component.empty();
        this.userNm = Component.empty();  // 清空用户名
    }
